...
TEXT2TEXT_MODEL_LIST: List[str] = ["flan-t5-small"]
...
st.subheader(f" Powered by :blue[` AWS Bedrock's Titan Large Model`] for text generation and :blue[{EMBEDDINGS_MODEL_LIST[0]}] for embeddings")
...
if user_input:
    ...
    if mode == MODE_TEXT2TEXT:        
        ...
        if resp.status_code != HTTP_OK:
            ...
            resp = resp.json()
            print("Response", resp['docs'])
            sources = [d['page_content'] for d in resp['docs']]
